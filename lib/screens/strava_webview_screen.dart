...
class _StravaWebViewScreenState extends State<StravaWebViewScreen> {
  ...
  void initState() {
    ...
    if (!kIsWeb) {
      ...
      // Don't automatically check for stored tokens - always show Connect button first
    }
  }
      ...
      if (token != null && !token.startsWith('Error:')) {
        try {
          ...
          try {
            ...
            // Handle any errors in counting activities
          }
          
          if (!mounted) return; // Final check before setState
          
          // Only show success if we actually imported activities
          if (total > 0) {
            setState(() {
              _isSuccess = true;
              _isLoading = false;
            ...
            if (!mounted) return;
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('No activities found. Please make sure you have activities in your Strava account.')),
            );
          }
        } catch (e, stack) {
  ...
  Future<void> _clearStoredTokens() async {
    ...
    await prefs.remove('strava_token_expires_at');
    await prefs.remove('strava_athlete_id');
    
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Stored tokens cleared. You can now connect to Strava again.')),
      );
    }
  }
                  ...
                  if (kIsWeb)
                    Column(
                      children: [
                        ...
                        TextButton(
                          onPressed: _clearStoredTokens,
                          child: const Text(
                            'Clear Stored Tokens (Debug)',
