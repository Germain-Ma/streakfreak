...
class RunProvider extends ChangeNotifier {
  ...
  List<Activity> _removeDuplicates(List<Activity> activities) {
    ...
    return activities.where((activity) {
      final stravaId = activity.id;
      if (stravaId == null || stravaId.isEmpty) {
        return true; // Keep activities without Strava ID
      }
      if (seenIds.contains(stravaId)) {
        return false; // Skip duplicate activities
      }
      seenIds.add(stravaId);
      return true;
    }).toList();
  }
} 